#!/usr/bin/env python3
"""
Lint transaction language files for errors and warnings.
...
from .validate import validate_schema, Fix

# Base directory for resolving imports - can be overridden via environment variable
_DEFAULT_PROTOCOL_BASE = Path(__file__).parent.parent / "protocol"
PROTOCOL_BASE = Path(os.environ.get("OMERTA_PROTOCOL_DIR", _DEFAULT_PROTOCOL_BASE))
...
def find_all_transactions(protocol_dir: Optional[Path] = None) -> list[Path]:
    """Find all transaction.omt files.

    Args:
        protocol_dir: Optional protocol directory. If not provided, uses PROTOCOL_BASE.
    """
    if protocol_dir is None:
        protocol_dir = PROTOCOL_BASE

    transactions_dir = protocol_dir / "transactions"
    if not transactions_dir.exists():
        # Try looking in current directory
        transactions_dir = Path.cwd() / "protocol" / "transactions"

    if not transactions_dir.exists():
        return []

    return sorted(transactions_dir.glob("*/transaction.omt"))


def main():
    parser = argparse.ArgumentParser(
        description="Lint transaction language files for errors and warnings."
    )
    parser.add_argument(
        "files",
        nargs="*",
