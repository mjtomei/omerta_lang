"""
PEG-based parser for the Omerta transaction language using Lark.

Uses a formal grammar definition (grammar.lark) and Lark's Earley parser
to produce AST nodes defined in ast.py.
"""
...
def _get_grammar_path() -> Path:
    """Get the path to the grammar file, handling both installed and development modes."""
    # First try package resources (for installed package)
    try:
        p = importlib.resources.files('omerta_lang').joinpath('grammar.lark')
        # as_file() returns a context manager for a concrete file path
        if hasattr(p, 'is_file') and p.is_file():
            return Path(str(p))
    except (TypeError, FileNotFoundError, AttributeError):
        pass

    # Fall back to file-relative path (for development)
    return Path(__file__).parent / "grammar.lark"


GRAMMAR_PATH = _get_grammar_path()
...
def parse(source: str) -> Schema:
    """Parse Omerta language source code into an AST Schema."""
    parser = get_parser()
...
def parse_file(path: str) -> Schema:
    """Parse an Omerta language file into an AST Schema."""
    with open(path) as f:
        return parse(f.read())
...
def load_transaction_ast(tx_path, base_dir=None) -> Schema:
    """
    Load a transaction language file with import resolution.
